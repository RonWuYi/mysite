...
class UserDataBase:
    ...
    def create_user(self, username, password):
        try:
            ...
            return 'create user failed: %s' %err
    ...
    def change_password(self,username, old_pwd, new_pwd):
        try:
            ...
            return 'change password failed: %s' %err
        else:
            return 'SUCCESS'
...
class User:
    ...
    def _validate_password(self, password):
        if not (7 <=len(password) <= 12):
            raise ValueError('pwd must be 7-12 characters long')
        if not self._validate_password_chars(password):
            raise ValueError('pwd must be combination of lowercase letters'
...
if __name__ == '__main__':
    actions = {'create': create_user, 'login': login, 'change-password': change_password,
                'help': help}
